...
function request({ method = "GET", path: reqPath, headers = {}, body }) {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error) => {
      reject(new Error(`网络请求失败：${error.code || error.message || "未知错误"}`));
    });
...
async function sign(session, captchaText) {
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    throw new Error(`签到请求失败：HTTP ${response.statusCode} ${response.body}`);
  }
...
async function getUserInfo(session) {
  ...
  if (response.statusCode < 200 || response.statusCode >= 300 || !result.data) {
    throw new Error(`获取用户信息失败：HTTP ${response.statusCode} ${response.body}`);
  }
...
if (require.main === module) {
  (async () => {
    try {
      ...
      console.log(`当前剩余流量：${formatRemainingTraffic(userInfo.raw.traffic)}`);
