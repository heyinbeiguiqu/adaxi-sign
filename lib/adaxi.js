...
function request({ method = "GET", path: reqPath, headers = {}, body }) {
  return new Promise((resolve, reject) => {
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new Error(`请求超时：${method} ${reqPath}`));
    });

    req.on("error", (error) => {
      reject(
        new Error(`网络请求失败：${error.code || error.message || "未知错误"}`),
      );
    });
...
async function login() {
  if (!username || !password) {
    throw new Error("请在 .env 中配置 ADAXI_USERNAME 和 ADAXI_PASSWORD");
  }
...
function formatUserInfo(userInfo) {
  return {
    ...
    remainingTraffic: formatRemainingTraffic(userInfo.traffic - userInfo.trafficked),
...
async function buildSignResult(session, signResult, captchaText, attempts) {
  return {
    signResult,
    captchaText,
    userInfo: formatUserInfo(await getUserInfo(session)),
    attempts,
  };
}
...
async function runSign(captchaText) {
  ...
  while (true) {
    ...
    const code = manualCaptcha || (await recognizeCaptcha(captchaBase64));
    manualCaptcha = undefined;

    const signResult = await sign(session, code);
    const normalizedCode = String(code).replace(/\s/g, "");

    if (signResult.message === "签到成功") {
      return buildSignResult(session, signResult, normalizedCode, attempt);
    }

    if (signResult.message === "验证码错误") {
      ...
      captchaRetries += 1;
      console.log(
        `第 ${attempt} 次签到：验证码错误（识别：${normalizedCode}），正在重试...`,
      );
      continue;
    }
...
if (require.main === module) {
  (async () => {
    try {
      const { signResult, captchaText, userInfo, attempts } = await runSign();

      console.log(`验证码识别结果：${captchaText}`);

      if (signResult.message === "签到成功") {
        console.log(
          `签到成功，奖励流量：${formatUsedTraffic(signResult.data)}（共尝试 ${attempts} 次）`,
        );
      } else {
        console.log(`签到失败：${signResult.message}`);
      }

      console.log(`当前剩余流量：${userInfo.remainingTraffic}`);
      console.log(`已使用流量：${userInfo.usedTraffic}`);
      console.log(`订阅链接：${userInfo.subLink}`);
    } catch (error) {
